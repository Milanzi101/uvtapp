...
const VisitorsScreen = () => {
  ...
  const fetchVisitHistory = async () => {
    ...
    try {
      const response = await axios.get(
        `https://4497-41-175-27-174.ngrok-free.app/api/VisitDetails/employeeCode/${employeeCodeIdFromPreviousScreen}`
      );
  ...
  const handleRequestVisit = () => {
    console.log('Navigating to VisitFormScreen');
    navigation.navigate('VisitFormScreen', { employeeCodeIdFromPreviousScreen });
  };
...
const styles = StyleSheet.create({
  ...
  addButton: { backgroundColor: 'black', padding: 15, borderRadius: 8, alignItems: 'center', marginTop: 16 },
